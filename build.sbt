...
lazy val commonSettings = commonSmlBuildSettings ++ publishTravisSettings ++ Seq(
  ...
  pgpSecretRing := file("$TRAVIS_BUILD_DIR/secring.asc"), // unpacked from secrets.tar.enc
  pgpPublicRing := file("$TRAVIS_BUILD_DIR/pubring.asc"), // unpacked from secrets.tar.enc
)
